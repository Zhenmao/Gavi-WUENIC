class VisTooltip {
  ...
  move(event) {
    let x = event.clientX - this.containerBox.x - this.tooltipBox.width / 2;
    if (x < 0) {
      x = 0;
    } else if (x + this.tooltipBox.width > this.containerBox.width) {
      x = this.containerBox.width - this.tooltipBox.width;
    }
    let y = event.clientY - this.containerBox.y - this.tooltipBox.height - 8;
    if (y < 0) {
      y = event.clientY - this.containerBox.y + 8;
    }
